...
real v, dpares, Npares, R1, R0, dilog, direal, pi
logical, allocatable ::check(:)
!------------------------------------------------------------
np=4226051                !particulas en la tabla
!-----------------defino los box-----------------------------
...
!	 if (dx<lg .and. dy<lg .and. dz<lg ) then
 	  distance=(((dx**2.)+(dy**2.)+(dz**2.))**0.5)
	  if (distance<lg .and. distance>lmin) then
!	  if ( ((dx**2.)+(dy**2.)+(dz**2.))**0.5<lg) then 
		distlog=(log10(distance))
		!print*,distlog
		write(11,*) ll(1,p), ll(2,p), ll(3,p)
...
do i=1,bines                    !int((lgnorm-lminnorm)/bin)
	...
	write(12,*) galaxybin(i), Npares, direal, (galaxybin(i)/Npares-1.)
